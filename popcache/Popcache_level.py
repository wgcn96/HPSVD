...
class Popcache:
    ...
    def curday_event_into_cube(self, is_validate=False):
        ...
        if is_validate:
            print("today hit rate is {}/{}".format(hit, count))
...
if __name__ == '__main__':
    ...
    group_count = 0
    result_list = []
    for cache_size_ratio in [10, 25, 50, 75, 100, 250, 500]:
    # for cache_size_ratio in [0.001, 0.0025, 0.005, 0.01]:
        total_each_day_item = np.zeros((6,), dtype=np.int32)
        total_each_day_hit = np.zeros((6,), dtype=np.int32)
        for name, group in (UIT.groupby([level])):
            ...
            if cache_size_ratio > 1:
                ...
                cache_size = int(items_num * cache_size_ratio)
            ...
            for i in range(24, 30, 1):
                ...
                total_each_day_item[i-24] += item_count
                total_each_day_hit[i-24] += hit
                popcache.update_feature()
                total_popularity = popcache.update_hypercube_estimate_value()
                popcache.update_cache_set()
                # popcache.print_cubes()
        ...
        print(each_day_hit_rate.round(4), each_day_hit_rate.mean().round(4))
        result_list.append(each_day_hit_rate.mean().round(4))
